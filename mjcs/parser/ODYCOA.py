from ..models import (ODYCOA, ODYCOAReferenceNumber, ODYCOAAttorney,
                      ODYCOADocument, ODYCOAInvolvedParty,
                      ODYCOACourtSchedule, ODYCOAJudgment)
...
class ODYCOAParser(CaseDetailsParser):
    inactive_statuses = [
        'Transferred',
        'Closed'
    ]
    ...
    def case(self, db, soup):
        case = ODYCOA(case_number=self.case_number)
    ...
    def reference_numbers(self, db, soup):
        ...
        while True:
            ...
            if not prompt_span:
                break
            ref_num = ODYCOAReferenceNumber(case_number=self.case_number)
            ...
            while True:
                ...
                if 'Attorney(s) for the' in subsection_name:
                    for span in subsection_table.find_all('span',class_='FirstColumnPrompt',string='Name:'):
                        attorney = ODYCOAAttorney(case_number=self.case_number)
    ...
    def judgments(self, db, soup):
        ...
        while True:
            ...
            judgment = ODYCOAJudgment(case_number=self.case_number)
    ...
    def court_schedule(self, db, soup):
        ...
        while True:
            ...
            schedule = ODYCOACourtSchedule(case_number=self.case_number)
    ...
    def document(self, db, soup):
        ...
        while True:
            try:
                ...
                break
            prev_obj = separator
            doc = ODYCOADocument(case_number=self.case_number)
