from ..models import (ODYCVCIT, ODYCVCITReferenceNumber, ODYCVCITDefendant,
...
class ODYCVCITParser(CaseDetailsParser):
    inactive_statuses = [
        'Citation Voided',
    ...
    def case(self, db, soup):
        case = ODYCVCIT(case_number=self.case_number)
    ...
    def reference_numbers(self, db, soup):
        ...
        while True:
            ...
            if not prompt_span:
                break
            ref_num = ODYCVCITReferenceNumber(case_number=self.case_number)
        ...
        while True:
            ...
            try:
                ...
                if party_type == 'Attorney for Defendant' and defendant_id:
                    party = ODYCVCITAttorney(case_number=self.case_number)
                    ...
                    party = ODYCVCITDefendant(case_number=self.case_number)
                else:
                    party = ODYCVCITInvolvedParty(case_number=self.case_number)
                ...
                while True:
                    ...
                    if subsection_name == 'Aliases':
                        for span in subsection_table.find_all('span',class_='FirstColumnPrompt'):
                            ...
                            alias_ = ODYCVCITAlias(case_number=self.case_number)
                            if type(party) == ODYCVCITDefendant:
                        ...
                        for span in subsection_table.find_all('span',class_='FirstColumnPrompt',string='Name:'):
                            attorney = ODYCVCITAttorney(case_number=self.case_number)
                            if type(party) == ODYCVCITDefendant:
            ...
            if type(party) != ODYCVCITDefendant:  # Defendant section doesn't separate parties with <hr>
    ...
    def charge_and_disposition(self, db, soup):
        ...
        existing_charges = db.query(ODYCVCITCharge).filter(ODYCVCITCharge.case_number == self.case_number).all()
        new_charges = []
        ...
        while True:
            ...
            charge = ODYCVCITCharge(case_number=self.case_number)
    ...
    def probation(self, db, soup):
        ...
        for span in t.find_all('span',class_='Prompt',string='Start Date:'):
            ...
            probation = ODYCVCITProbation(case_number=self.case_number)
    ...
    def restitution(self, db, soup):
        ...
        if len(list(t.stripped_strings)) > 0:
            for row in t.find_all('tr'):
                restitution = ODYCVCITRestitution(case_number=self.case_number)
